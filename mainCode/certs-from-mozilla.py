...
if which('ar') is None and not os.path.isfile('./ar') and not os.path.isfile('C:/Program Files (x86)/Arduino/hardware/esp8266com/esp8266/tools/xtensa-lx106-elf/xtensa-lx106-elf/bin/ar.exe'):
...
if which('openssl') is None and not os.path.isfile('./openssl') and not os.path.isfile('C:/Program Files/OpenSSL-Win64/bin/openssl.exe'):
...
for i in range(0, len(pems)):
    certName = "D:/Personel/PersonelProject/RobotProje/arduinoRemoteControlCar/mainCode/data/ca_%03d.der" % (idx);
    thisPem = pems[i].replace("'", "")
    print(names[i] + " -> " + certName)
    ssl = Popen(['C:/Program Files/OpenSSL-Win64/bin/openssl.exe','x509','-inform','PEM','-outform','DER','-out', certName], shell = False, stdin = PIPE)
    pipe = ssl.stdin
